...
PanomaxImages::PanomaxImages(QObject* parent) :
    QObject{parent}, m_recentImageModel{"image://" + PanomaxImageProvider::providerId + "/"}
{
  ...
  connect(&m_recentUpdateTimer, &QTimer::timeout, this, &PanomaxImages::updateRecentImageInfo);
  m_recentUpdateTimer.setInterval(1min);
  updateRecentImageInfo();
  m_recentUpdateTimer.start();
}

void PanomaxImages::setPanomaxImageProvider(PanomaxImageProvider* imageProvider)
{
  Q_ASSERT(m_imageProvider == nullptr);
...
void PanomaxImages::updateRecentImageInfo()
{
  ...
  QNetworkRequest req{QString{"https://api.panomax.com/1.0/cams/%1/images/day"}.arg(m_camId)};
  m_recentReply = m_netManager.get(req);

  connect(m_recentReply, &QNetworkReply::finished, this, &PanomaxImages::recentImageInfoReceived);
}
...
void PanomaxImages::downloadRecentImage(PanomaxImageInfo::Resolution resolution)
{
  ...
  auto iter =
      std::ranges::find_if(m_recentImageInfo.imageSizes(), [resolution](const PanomaxImageInfo::ImageSize& imageSize) {
  ...
  m_recentImageDownload.imageDownloader =
      std::make_unique<PanomaxImageDownloader>(m_recentImageInfo.recentImage(), resolutionString, iter->numTiles);

  connect(m_recentImageDownload.imageDownloader.get(),
          &PanomaxImageDownloader::finished,
          this,
...
void PanomaxImages::recentImageDownloadFinished()
{
  ...
  if (m_recentImageDownload.imageDownloader->error() != QNetworkReply::NoError)
  {
    qWarning() << "recent image download failed";
    return;
  }

  m_imageProvider->setRecentImageTile(m_recentImageDownload.imageDownloader->imageTiles());
  m_recentImageModel.updateTiles(m_recentImageDownload.imageDownloader->imageTiles(),
